...
class MultiFactorRollingOLS:
    ...
    def get_rolling_beta(self):
        rolling_dfs = self.rolling_dfs

        date = list(self.rolling_dfs.values())[0]['date']
        ...
        independent_ticker_list = []
        for ticker, df in rolling_dfs.items():
            beta = df['beta']
            rolling_beta_df[f"{ticker}-rolling-beta"] = beta
            independent_ticker_list.append(f"{ticker}-rolling-beta")


        rolling_beta_df["date"] = pd.to_datetime(rolling_beta_df["date"])

        #rolling_beta_df = rolling_beta_df.sort_values("date").set_index("date")

        rolling_beta_df = rolling_beta_df.dropna().reset_index(drop=True)
