...
def factors(
    ...
    factor_source: str = typer.Option('french', "--factor_source", "-fs", help = "Choose between Fama-French factors or unofficial ETF proxy factors. Options: 'french', 'etf'"),
    start_date: str | None = typer.Option(None, "--start_date", "-s", help="Start date (YYYY-MM-DD) of factor decomposition window (optional)"),
    end_date:str | None = typer.Option(None, "--end_date", "-e", help="End date (YYYY-MM-DD) of factor decomposition window (optional)"),
    return_type:str = typer.Option('simple', "--return_type", "-r", help="Returns calculation methodology. Options: 'simple' or 'log' (optional, defaults to 'simple')"),
